...
class ImporterForm extends FormBase {
    ...
    if ($entity_type = $form_state->getValue('entity_type')) {
      ...
      if ($options = $this->getEntityTypeBundleOptions($entity_type)) {
        $form['importer']['entity_type_bundle'] = [
          ...
          '#options' => $options,
          //'#options' => ["user_revision" => "user_revision"],
          '#required' => TRUE,
          '#weight' => 5,
        ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    \Drupal::logger('$plugin_id')->notice(print_r($form_state->getUserInput()['plugin_id'], true));
    \Drupal::logger('$csv_parse')->notice(print_r($csv_parse, true));
    \Drupal::logger('$csv')->notice(print_r($csv, true));
    \Drupal::logger('$entity_type')->notice(print_r($entity_type, true));
    \Drupal::logger('$entity_type_bundle')->notice(print_r($entity_type_bundle, true));
    ...
    if ($required = $this->getEntityTypeMissingFields($entity_type, $entity_fields['required'], $csv_parse)) {
      ...
      $count = (count($csv_parse) - 1);
      array_push($csv_parse[0], 'field_informacion_valida');
      for($i = 0; $i <= (int)$count; $i++) {
        if ($i > 0) {
          $csv_parse[$i][]['field_informacion_valida'] = 1;
        } // end if
      } // end foreach
